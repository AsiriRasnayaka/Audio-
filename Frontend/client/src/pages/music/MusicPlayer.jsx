...
import { getSongById, playSong } from "../../store/slices/songSlice";
...
export default function MusicPlayer() {
  ...
  const { loading, currentSong, error } = useSelector((state) => state.song);
  ...
  const fetchAndPlaySong = async () => {
    try {
      await dispatch(getSongById(id)).unwrap(); // fetch song
      await dispatch(playSong(id)).unwrap();    // increment play count
    } catch (err) {
      console.error("Error fetching or playing song:", err);
    }
  };

  if (id) fetchAndPlaySong();
}, [id, dispatch]);
  ...
  return (
    <div className="min-h-screen pb-20 bg-gradient-to-br from-slate-800 to-gray-900 py-6 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div className="p-4 sm:p-6 md:p-8">
          ...
          <AddToPlaylistButton songId={currentSong._id} />
          {/* Progress Bar */}
          <div className="mb-4">
            <input
              type="range"
